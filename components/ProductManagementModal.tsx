...
const ProductManagementModal: React.FC<ProductManagementModalProps> = ({ isOpen, onClose, products, onUpdateImage, onUpdateDescription, onUpdateStory }) => {
  ...
  const handleStartImageGeneration = (product: Product) => { resetAllStates(); setGeneratingImgFor(product.id); setImgGenPrompt(`A professional, clean, high-resolution product photograph of a ${product.name}, on a neutral studio background, with soft lighting.`); };
