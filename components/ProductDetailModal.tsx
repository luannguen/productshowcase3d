...
const ProductDetailModal: React.FC<ProductDetailModalProps> = ({ isOpen, product, onClose, onAddToCart, onBuyNow }) => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
        ...
        case 'details':
        default:
            // FIX: The `variants` prop can only be used on `motion` components. Changed `<p>` to `<motion.p>`.
      ...
      <div className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        <div className="w-full flex flex-col gap-3">
             <div className="w-full aspect-square bg-[var(--background-tertiary)] rounded-[var(--border-radius)] overflow-hidden flex items-center justify-center shine-effect">
                <AnimatePresence mode="wait"><motion.div key={activeMedia} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full h-full">{activeMedia === 'image' ? (<motion.img src={product.imageUrl} alt={product.name} className="w-full h-full object-cover" layoutId={`product-image-${product.id}`} whileHover={{ scale: 1.05 }} transition={{ duration: 0.3 }} />) : (<video src={product.videoUrl} className="w-full h-full object-cover" controls autoPlay loop />)}</motion.div></AnimatePresence>
             </div>
        ...
        <motion.div className="flex flex-col" variants={parentVariants} initial="hidden" animate="visible">
          <motion.h2 variants={childVariants} className="text-3xl lg:text-4xl font-bold bg-gradient-to-r from-[var(--header-gradient-from)] to-[var(--header-gradient-to)] text-transparent bg-clip-text" style={{ fontFamily: 'var(--font-family)' }}>{product.name}</motion.h2>
          ...
          <motion.div variants={childVariants} className="mt-auto pt-6 flex flex-col sm:flex-row gap-3">
            <motion.button onClick={handleAddToCartClick} className="w-full px-6 py-3 font-bold rounded-[var(--border-radius)] transition-colors duration-300 flex items-center justify-center gap-2 text-lg shadow-lg overflow-hidden relative border-2 border-[var(--primary-accent)]" animate={isAdded ? "added" : "idle"} variants={{ idle: { backgroundColor: 'transparent', color: 'var(--primary-accent)' }, added: { backgroundColor: 'var(--primary-accent)', color: '#FFFFFF' } }} aria-live="polite">
              <AnimatePresence>{!isAdded ? (<motion.span key="add" className="flex items-center gap-2" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }}><CartIcon className="w-6 h-6" /> Add to Cart</motion.span>) : (<motion.span key="added" className="flex items-center gap-2" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }}><CheckIcon className="w-6 h-6" /> Added</motion.span>)}</AnimatePresence>
            </motion.button>
            <motion.button onClick={handleBuyNowClick} className="w-full px-6 py-3 font-bold rounded-[var(--border-radius)] transition-colors duration-300 flex items-center justify-center gap-2 text-lg shadow-lg bg-[var(--primary-accent)] text-white hover:bg-[var(--primary-accent-hover)]">Buy Now</motion.button>
          </motion.div>
        </motion.div>
      </div>
    </BaseModal>
